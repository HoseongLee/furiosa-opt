...
fn basic_transpose<'l, const T: Tu>(
    input: CollectTensor<'l, T, i8, m![1], m![1 # 2], m![P], m![C, D], m![E # 32]>,
) -> TransposeTensor<'l, T, i8, m![1], m![1 # 2], m![P], m![C, E], m![D # 32]> {
    input.transpose()
}
# 
# let mut ctx = Context::acquire();
# 
# let c: CollectTensor<'_, _, i8, m![1], m![1 # 2], m![P], m![C, D], m![E # 32]> = CollectTensor::new(&mut ctx.main, Tensor::uninit());
...
# let mut ctx = Context::acquire();
# 
# let c: CollectTensor<'_, _, i8, m![1], m![1 # 2], m![P], m![A], m![B # 32]> = CollectTensor::new(&mut ctx.main, Tensor::uninit());
...
# let mut ctx = Context::acquire();
# 
# let c: CollectTensor<'_, _, i8, m![1], m![1 # 2], m![P], m![B, C, D], m![E # 32]> = CollectTensor::new(&mut ctx.main, Tensor::uninit());
...
# let mut ctx = Context::acquire();
# 
# let c: CollectTensor<'_, _, bf16, m![1], m![1 # 2], m![P], m![C, D], m![E # 16]> = CollectTensor::new(&mut ctx.main, Tensor::uninit());
...
# let mut ctx = Context::acquire();
# 
# let c: CollectTensor<'_, _, i4, m![1], m![1 # 2], m![P], m![B, C], m![E # 64]> = CollectTensor::new(&mut ctx.main, Tensor::uninit());
...
# let mut ctx = Context::acquire();
# 
# let c: CollectTensor<'_, _, f32, m![1], m![1 # 2], m![P], m![B, D], m![E # 8]> = CollectTensor::new(&mut ctx.main, Tensor::uninit());
