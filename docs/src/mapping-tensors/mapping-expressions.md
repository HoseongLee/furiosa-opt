...
This page defines its constructors and equivalence rules.
...
- for every buffer index `i` and tensor index `ti` where `E::map(i) = Some(ti)`,
- the `i`-th element stores the value of tensor \\(T\\) at index `ti`.

Device tensors such as `HbmTensor` and `DmTensor` have more complex layouts spanning multiple mapping expressions; see [Spatial and Temporal Dimensions](./spatial-temporal-dimensions.md) for details.
...
> Formal definition: `size_S($(e1:n1, ..., ed:nd)) = 1 + sum_k((size_S(ek) - 1) * nk)`.
> The mapping `S, $(e1:n1, ..., ed:nd) |- si ~ ti` holds if there exist `si1...sid, ti1...tid` such that for all `k`: `S, ek |- sik ~ tik`, `si = sum_k(sik * nk)`, and `ti = sum_k(tik * nk)`.
>
> Linear combinations can encode outer sum: `e1 * e2` is equivalent to `$(e1 : size_S(e2), e2 : 1)`.
> However, outer sum is preferred because it's more resilient to axis reordering.
> Changing `e1 * e2` to `e2 * e1` doesn't require manual stride updates.
...
> For example, space index `4` maps to `{4_N}`, `{2_N, 1_F}`, and `{2_F}` simultaneously.
> This illustrates the non-one-to-one nature of `(S, e).maps(si, ti)`.
